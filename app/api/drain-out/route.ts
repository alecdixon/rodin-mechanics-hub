...
function buildFriendlyEmailError(rawMessage: string) {
  ...
  if (
    ...
    return {
      ...
      fix:
        "Open a browser, log into the Gmail account used in GMAIL_USER, open Gmail, approve any security warning, then redeploy Vercel. If it keeps happening, switch this route to Resend instead of Gmail SMTP.",
    };
  }
  ...
  if (
    ...
    return {
      error: "The SMTP login failed before the email could be sent.",
      likely_cause:
  ...
  if (
    ...
    return {
      error: "The app could not connect to Gmail SMTP.",
      likely_cause:
